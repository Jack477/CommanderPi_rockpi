...
if os.path.exists(path+'/cpi.config'):
	...
	config['DEFAULT'] = {
		'version'     : cpi_version,
		'; debug_level # 0=none 1=custom 2=general 3=debug' : '',
		'debug_level' : cpi_debug_lvl,
		'; debug # turn on console debug output' : '',
		'debug'       : cpi_debug,
		'; auto_check # dont check for update at startup' : '',
		'auto_check'  : cpi_auto_check,
		'; auto_update # auto install update at startup' : '',
		'auto_update' : cpi_auto_update,
		'; color_mode # use dark theme, 0=light' : '',
...
### setCPiConfigValue(string, string, (optional)string)
def setCPiValue(section, value, CPiKey='DEFAULT'):
	config.set(CPiKey, section, value)


### Use it when update arrives
def updateCPiConfig():
